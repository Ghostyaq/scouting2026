...
all_sb_data <- read_csv("prior_ridge/data/all_epas_for_all_teams.csv")
all_tba_data <- read_csv("prior_ridge/data/real_scores.csv")
...
calc_by_match <- function(curr, design, response, priors, event_key, tba_data, sb_data){
    ...
    attempt <- tryCatch({
        ...
        mer <- prior_ridge(design_l, response_l, lambda_opt, priors)
        mer <- round(mer, 2)
...
acc_comp_event <- function(event_key){
    print(event_key)
    sb_data <- all_sb_data |> filter(event == event_key) |> unique()
