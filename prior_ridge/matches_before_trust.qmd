...
pridge_lambda_cv <- function(
    ...
    mses <- tryCatch({
        foreach::foreach(
            lambda = grid, .export = c("pridge_loocv", "prior_ridge"), .combine = 'c', .packages = 'scoutR'
        ) %dopar% {
...
min_mses <- sapply(5:10, function(i){
    mses <- pridge_lambda_cv(design[1:(2*i), ], response[1:(2*i)], epas, grid)
    min(mses)
})
