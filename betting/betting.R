...
winner_is_red <- filter(bets, scout_initials == "DS")$prediction == "Red"

bets <- mutate(bets, prediction_is_red = prediction == "Red", winner_is_red = winner_is_red[match_number]) |> 
    select(scout_initials, match_number, prediction_is_red, winner_is_red, point_bet)
