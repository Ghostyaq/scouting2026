...
pridge_hopper_offline <- function(event_key) {
    ...
    mean_cycles <- raw |> 
        group_by(team) |>
        summarize(cycles = mean(auto_cycles/10 + 
                                    num_cycles + 
                                    num_cycles_tenths/10))
    
    cols_to_keep <- colSums(design != 0) >= 2
    mean_cycles <- mean_cycles[cols_to_keep, ]
    design <- design[, cols_to_keep]
    auto_priors <- auto_priors[cols_to_keep]
    tele_priors <- tele_priors[cols_to_keep]
    priors <- (auto_priors + tele_priors)/mean_cycles$cycles
    ...
    priors_df <- data.frame(team = colnames(design), 
                            HOPpeR = fuel, 
                            total_fuel = fuel*mean_cycles$cycles, 
                            mean_cycles = round(mean_cycles$cycles,2))
    ...
    pridge <- select(pridge, -mean_cycles)
    
    pridge <- pridge |> mutate(HOPpeR = priors_df$HOPpeR, 
                               cycles = priors_df$mean_cycles)
